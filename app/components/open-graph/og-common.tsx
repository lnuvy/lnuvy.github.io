...
const OGCommon = (props: OGCommonProps) => {
  ...
  return (
    <Head>
      ...
      {/* title & description */}
      <title>
        {title ?? `${HEAD_COMMON_INFO.NAME} | ${HEAD_COMMON_INFO.CATCHPHRASE}`}
      </title>
      <meta
        name={OG['description']}
      ...
      <link
        rel={OG['author']}
        href={author ?? HEAD_COMMON_INFO.NAME}
        key={OG['author']}
      />
      {/* openGraph */}
      <meta
      ...
      <meta
        property={OG['og:url']}
        content={url ?? HEAD_COMMON_INFO.URL}
        key={OG['og:url']}
      />
      <meta
        property={OG['og:site_name']}
        content={title ?? HEAD_COMMON_INFO.NAME}
        key={OG['og:site_name']}
      />
      <meta
      ...
      <meta
        ...
        content={
          image
            ? `https://www.ffeed.me/_next/image?url=${image}&w=450&q=100`
            : HEAD_COMMON_INFO.IMAGE
        }
        key={OG['og:image']}
      />
      {/* twitter */}
      ...
      <meta
        ...
        content={image ?? HEAD_COMMON_INFO.IMAGE}
        key={OG['twitter:image']}
      />
      {ogRender && ogRender()}
    </Head>
  )
